...
QUnit.test("Matchmaker: Connection message is recieved correctly", function (assert) {
	...
	assert.deepEqual(messages[0], expectedMessage, "The spectate message should" + " have been sent after changing the connection.");
});
...
QUnit.test("Matchmaker: Pending game message is recieved correctly", function (assert) {
	...
	assert.equal(showString[0], "Waiting for: player1,player2", "The string passed" + " into showWaitingString should be correct.");
});
...
QUnit.test("Matchmaker: Registered message is recieved correctly", function (assert) {
	...
	assert.deepEqual(gameMessenger.storedMessage, registeredMessage, "The registered" + " message should have been stored in gameMessenger's variable storedMessage.");
});
