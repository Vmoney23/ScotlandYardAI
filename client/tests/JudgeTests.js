...
QUnit.test("Judge: Ready message received correctly.", function (assert) {
	...
	assert.deepEqual(guiConnector.testContainer.setTicketView[0], [true, true],
...
QUnit.test("Judge: Notify turn message received correctly.", function (assert) {
	...
	assert.deepEqual(guiConnector.testContainer.startTurn[0],
		notifyTurnMessage,
		"The decoded notify turn message should have been passed" + " directly to the startTurn method in GUIConnector.");
});
...
QUnit.test("Judge: Notify message received correctly - 1.", function (assert) {
	...
	assert.equal(guiConnector.testContainer.animatePlayer.length,
		2,
		"The animatePlayer method in GUIConnector should have been called" + " as a MoveTicket was in the notify message.");
	assert.equal(guiConnector.testContainer.animatePlayer[0],
		"Blue",
		"The colour of the player should stay the same when passed to the" + " animatePlayer method.");
	assert.equal(guiConnector.testContainer.animatePlayer[1],
		14,
		"The target of the player should stay the same when passed to the" + " animatePlayer method.");

	assert.ok(guiConnector.testContainer.removeTicket.length == 2,
		"The removeTicket method in GUIConnector should have been called" + " as a MoveTicket was in the notify message.");
	assert.equal(guiConnector.testContainer.removeTicket[0],
		"Blue",
		"The colour of the player should stay the same when passed to the" + " removeTicket method.");
	assert.equal(guiConnector.testContainer.removeTicket[1],
		"Taxi",
		"The ticket of the player should stay the same when passed to the" + " removeTicket method.");

	assert.ok(guiConnector.testContainer.updateTicketView.length == 0,
		"The updateTicketView method in GUIConnector should not have been" + " called as the colour of the player was not Black");
});
...
QUnit.test("Judge: Notify message received correctly - 2.", function (assert) {
	...
	assert.ok(guiConnector.testContainer.removeTicket.length == 2,
		"The removeTicket method in GUIConnector should have been called" + " as a MoveDouble was in the notify message.");
	assert.equal(guiConnector.testContainer.removeTicket[0],
		"Black",
		"The colour of the player should stay the same when passed to the" + " removeTicket method.");
	assert.equal(guiConnector.testContainer.removeTicket[1],
		"Double",
		"The ticket of the player should be 'Double' when passed to the" + " removeTicket method.");
	assert.ok(guiConnector.testContainer.animatePlayer.length == 0,
		"The animatePlayer method in GUIConnector should not have been called" + " as a MoveDouble was in the notify message.");
	...
	assert.ok(guiConnector.testContainer.updateTicketView.length == 2,
		"The updateTicketView method in GUIConnector should have been" + " called as the colour of the player was Black");
	assert.equal(guiConnector.testContainer.updateTicketView[0],
		"Taxi",
		"The ticket of the player should stay the same when passed to the" + " updateTicketView method.");
	assert.equal(guiConnector.testContainer.updateTicketView[1],
		"14",
		"The target of the player should stay the same when passed to the" + " updateTicketView method.");
...
QUnit.test("Judge: Notify message received correctly - 3.", function (assert) {
	...
	gameMessenger.handleMessage(JSON.stringify(notifyMessage));
	assert.ok(guiConnector.testContainer.animatePlayer.length == 0,
		"The animatePlayer method in GUIConnector should not have been called" + " as a MovePass was in the notify message.");

	assert.ok(guiConnector.testContainer.removeTicket.length == 0,
		"The removeTicket method in GUIConnector should not have been called" + " as a MovePass was in the notify message.");
...
QUnit.test("Judge: Game over message received correctly.", function (assert) {
	...
	assert.deepEqual(guiConnector.testContainer.setGameOver[0],
		gameOverMessage,
		"The decoded game over message should have been passed directly" + " to the setGameOver method in GUIConnector.");
});
