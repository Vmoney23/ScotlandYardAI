...
var SetUpView = function (viewId, joinFormId, spectateFormId, matchmakerFormId, aiFormId, idFormId, mainPaneId, waitingPaneId, idPaneId, listId) {
	...
	if (this.matchmakerForm.addEventListener) this.matchmakerForm.addEventListener("submit", function (event) {
		tryMatchmaker(event, sendEnquire);
	}, false);
	else if (this.matchmakerForm.attachEvent) this.matchmakerForm.attachEvent('onsubmit', function (event) {
		tryMatchmaker(event, sendEnquire);
	});
