...
var DropDown = function (canvas, ticketImgs) {
	...
	if (canvas.addEventListener) {
		canvas.addEventListener("mousedown", function (event) {
			self.mouseDown(event, canvas)
		}, false);
		canvas.addEventListener("mousemove", function (event) {
			self.mouseMoved(event, canvas)
		}, false);
		canvas.addEventListener("mouseout", function (event) {
			self.mouseOut(event, canvas)
		}, false);
	} else if (canvas.attachEvent) {
		canvas.attachEvent("onmousedown", function (event) {
			self.mouseDown(event, canvas)
		});
		canvas.attachEvent("onmousemove", function (event) {
			self.mouseMoved(event, canvas)
		});
		canvas.attachEvent("onmouseout", function (event) {
			self.mouseOut(event, canvas)
		});
	}
};
...
DropDown.prototype.mouseDown = function (event, canvas) {
	var mouseLoc = this.positionRelativeTo({
		x: event.clientX,
		y: event.clientY
	}, canvas);
	if (this.moves) {
